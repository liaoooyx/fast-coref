...
class MentionProposalModule(nn.Module):
    ...
    def _build_model(self, hidden_size: int) -> None:
        ...
        self.span_width_embeddings = nn.Embedding(mention_params.max_span_width, mention_params.emb_size)
        self.span_width_prior_embeddings = nn.Embedding(mention_params.max_span_width, mention_params.emb_size)
    ...
    def get_span_embeddings(self, encoded_doc: Tensor, ment_starts: Tensor, ment_ends: Tensor) -> Tensor:
        ...
        span_width_indices = torch.clamp(ment_ends - ment_starts, max=self.config.mention_params.max_span_width - 1)
    ...
    def get_mention_width_scores(self, cand_starts: Tensor, cand_ends: Tensor) -> Tensor:
        ...
        span_width_idx = torch.clamp(cand_ends - cand_starts, max=self.config.mention_params.max_span_width - 1)
    ...
    def get_flat_gold_mentions(self, clusters: List, num_tokens: int, flat_cand_mask: Tensor) -> Tensor:
        ...
        gold_ments = torch.zeros(num_tokens, self.config.mention_params.max_span_width, device=self.device)
    ...
    def get_candidate_endpoints(self, encoded_doc: Tensor, document: Dict) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        cand_starts = torch.unsqueeze(torch.arange(num_words, device=self.device), dim=1).repeat(
            1, self.config.mention_params.max_span_width
        )
        ...
        subtoken_map: Tensor = torch.tensor(
            document["subtoken_map"] + [-1] * (self.config.mention_params.max_span_width + 1),
            device=self.device,
        )
    ...
    def pred_mentions(self, document: Dict, encoded_doc: Tensor, ment_threshold=0.0) -> Dict:
        ...
        # Newly added for active learning
        output_dict["mention_logits"] = mention_logits
